...
using Maps.Data.OpenStreetMap.Tests;
...
namespace Maps.Data.Tests.Compilation
{
    [TestFixture]
    public class TiledFeatureTaskTests
    {
        ...
        [Test]
        public void TestStartMethodOsmSingleThread()
        {
            RunCompile(1);
        }

        [Test]
        public void TestStartMethodOsmFourThreads()
        {
            RunCompile(4);
        }

        private static void RunCompile(int threads)
        {
            using (var pgconn = new NpgsqlConnection(PostgreOsmConnectionString))
            {
                using (var provider = new OpenStreetMapProvider(pgconn).FeatureProvider)
                {
                    using (var master = provider.FeatureSource())
                    {
                        using (var maps = new MapsTestProvider())
                        {
                            using (var features = maps.FeatureProvider)
                            {
                                ...
                                {
                                    //var box = new GeodeticBox2d(new Geodetic2d(48.3133d, 10.4109d), new Geodetic2d(49.1548d, 12.5670d));
                                    ...
                                    var simplifier16 = new FeatureSimplifier(new PassThroughSimplifier());
                                    //var simplifier16 = new FeatureSimplifier(
                                    //    new RamerDouglasPeukerSimplifier(simplificationFactor));
